...
def converter_para_brasil(horario_utc_naive: datetime) -> str:
    """Recebe um datetime naive (assumido como UTC) e retorna ISO string em horário de Brasília."""
    return (
        horario_utc_naive.replace(tzinfo=timezone.utc)
...
class Leitura(Base):
    ...
    horario = Column(DateTime, default=agora_utc)
...
def debug_timezone():
    ...
    Remova depois de confirmar que o fuso horário está funcionando em produção.
    """
    agora = datetime.now(timezone.utc)
    try:
        convertido = agora.astimezone(FUSO_BRASIL)
        return {
            "utc_agora": agora.isoformat(),
            "brasil_agora": convertido.isoformat(),
            "diferenca_horas": (agora.replace(tzinfo=None) - convertido.replace(tzinfo=None)).total_seconds() / 3600,
            "zoneinfo_ok": True
        }
    except Exception as e:
        return {
    ...
    return [
        {
            ...
            "horario": converter_para_brasil(leitura.horario),
